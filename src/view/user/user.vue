    ...
    class="margin-20"
      ...
      <el-dialog title="分配角色" :visible.sync="grantDialogFormVisible">
        <el-form :model="grantForm" label-width="120px">
          <el-form-item label="请选择用户" prop="username">
...
<script>
import {getUserList,changeUserState,addUser,getUserById,submitUser,deleteUser,getRoleList,grantUserRole} from '@/api'
export default {
    data() {
      return {
        loading:true,
    ...
    methods: {
      ...
      editUserSubmit(formName){
        this.$refs[formName].validate(valide=>{
          if(valide){
            submitUser(this.editForm).then(res=>{
              if(res.meta.status===200){
                this.$message({
                  type:'success',
                  message:'编辑成功'
